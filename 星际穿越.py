...
while page_count < max_pages:
    ...
    if page_count % 5 == 0 and all_comments:
        try:
            ...
            if not os.path.exists(r"C:\Users\samsung\Desktop\实验\星际穿越脏.csv"):
                df.to_csv(r"C:\Users\samsung\Desktop\实验\星际穿越脏.csv", index=False, encoding='utf-8-sig')
                print(f'创建本地CSV文件并保存 {len(all_comments)} 条数据')
            else:
                df.to_csv(r"C:\Users\samsung\Desktop\实验\星际穿越脏.csv", mode='a', header=False, index=False, encoding='utf-8-sig')
